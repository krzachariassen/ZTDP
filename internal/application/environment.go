...
func NewEnvironmentService(g *graph.GlobalGraph) *EnvironmentService {
	return &EnvironmentService{Graph: g}
}
...
func (s *EnvironmentService) CreateEnvironmentFromContract(ctx context.Context, env *contracts.EnvironmentContract) (interface{}, error) {
	if env == nil {
...
func (s *EnvironmentService) ListAllowedEnvironmentsAsData(appName string) ([]map[string]interface{}, error) {
	// Use existing logic to get contracts
...
func (s *EnvironmentService) UpdateAllowedEnvironments(appName string, envs []string) error {
	appNode, err := s.Graph.GetNode(appName)
	if err != nil || appNode == nil || appNode.Kind != "application" {
		return errors.New("application not found")
	}
...
func (s *EnvironmentService) AddAllowedEnvironments(appName string, envs []string) error {
	appNode, err := s.Graph.GetNode(appName)
	if err != nil || appNode == nil || appNode.Kind != "application" {
		return errors.New("application not found")
	}
